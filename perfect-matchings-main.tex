...
\ari{Lemma 5.1 should spit out a single $\Eps$ that we use later.}

\begin{lemma}\label{lemma:partition}Given a $\EnDeeLambda$ graph $\Graph = (V, E)$, there exists a small constant $\Eps \in (0,1)$ and a partition of $V$ into disjoint sets $A$ and $B$, such that, we have for every $v \in V$, $|\Neighbourhood{\EdgesShort}{v} \cap A| \leq 2\epsilon d$.
...
As $\Graph$ is $d$-regular, $\Mean{\vec{X}}{Y_v} = \epsilon d$, and observe that the dependency graph (Definition \ref{def:dep-graphs}) of events $\{ E_v \}_{v \in V}$ has max-degree at most $d$.
...
Let $\dApproxLower \Def \dApproxExpandedLower$, and $H$ be the worst case hard instance on $t = \lceil\epsilonA \frac{\na}{\log \na}\rceil$ vertices, where $0 < \epsilonL < \epsilonA <1$ are the constants from the partitioning Lemma \ref{lemma:bipartitie-is-non-blocking}. 
\ari{Define $\sigma$}.
...
\begin{proof}
Let $\PartitionEps = 2\Eps$, where $\Eps$ is as defined in Lemma \ref{lemma:partition} (Figure \ref{subfig:partitionA}).
Define $\tilde{d} = d - \frac{d\PartitionEps}{2}$ and let $\tilde{G}=(\tilde{V}, \tilde{E})$ be short hand for $\Graph[V \setminus \Embedding{\Subdivision{H}{\sigma}}]$.
Assume that the edge connectivity $\EdgeConnectivity{\tilde{G}}= \tilde{d}$. Then we show using the Tutte criterion that $\tilde{G}$ admits a perfect matching.
Pick arbitrary $S \subseteq \Vertices{\tilde{G}}$.
By the guarantees of Lemma \ref{lemma:partition}, the minimal degree of $\tilde{G}$ is $\tilde{d}$. 
...
Let $\tilde{n} = |\tilde{V}|$.
Pick arbitrary $U \subset \tilde{V} \subset V$ such that $|U| \Def u \leq \tilde{n}/2$.
...
To construct $\tilde{G}$ we delete only a small fraction of edges for each vertex $v \in V$, such that the degree of every vertex $v \in \tilde{V}$ is at least $\tilde{d}$.
Therefore, the number of edges across $U$ and $\Complement{U}$ based on $E$ we might have to delete to construct $\tilde{G}$ is at most $\frac{\PartitionEps d}{2}\cdot u$.
...
\eqref{eq:lambda-assumption} comes from the assumption that $\ExpansionFactor{G} \leq d - \PartitionEps d - 2$.
