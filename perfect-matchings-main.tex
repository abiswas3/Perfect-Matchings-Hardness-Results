...
In their seminal work, \citet{cook1979relative} observed that the above question could only be made mathematically rigorous by first describing the format in which the proof is written, and the rules for checking it.
Each set of rules for writing and checking a proof defines a proof system.
Therefore, more precisely, the question should be restated as ``\textit{Given a true statement $A$ and a proof system $S$, what is the length of the shortest proof $\Proof \in S$ that $A$ is true?}''
...
Thus, as an intermediate step towards making partial progress, the research community has invested a significant amount of time and energy in proving size lower bounds for more restricted proof systems \citep{blake1937canonical,razborov1998lower, impagliazzo1999lower, alekhnovich2001lower, buss1999linear}.
One example of a simple but inefficient\footnote{When we say inefficient, we mean that proofs of true claims are long in the worst case.} proof system is that of truth tables i.e. given a propositional formula $\PropFormula$ that is a tautology, a valid proof is simply the truth table for the formula.
Given a formula on $n$ variables, the size of the proof is $2^n$ and it is easy to see that we cannot use shorter proofs.
Truth tables are very from being general proof systems, and thus, the research community has considered increasingly more expressive proof systems such as resolution, bounded depth Frege proof systems, Nullstellensatz etc.
...
In this work, we focus on a semi-algebraic proof systems\footnote{Write a remark about how these also extend to bounded Frege \highlight{TODO}.} like polynomial calculus (PC) \citep{alekhnovich2004pseudorandom} and sum of squares (SOS) proof system \citep{parrilo2000structured, boazCourse}.
...
Although algebraic by definition, one can use semi-algebraic proof systems to capture constraints of combinatorial objects like graphs.
...
In this work we are interested in the complexity of PC and SOS refutations for $\Card{G, 1^{|V|}}$  when $|V|$ is odd\footnote{\highlight{We are interested in something more general but studying this suffices.}}.
...
Thus, the main contribution of \citet{Austrin_2022} is a new but relatively complex embedding theorem, and a distinctive feature of their result is that the embedding relies on randomness used to sample the regular graph.
Thus, they give an average case hardness result, as opposed to a worst case lower bound. 
Furthermore, these techniques do not apply when the host graph $G$ is deterministic, as in the case of deterministic constructions of $\EnDeeLambda$-graphs.
...
It should be noted that as pseudorandom graph is \emph{also} a random graph with high probability, our result subsumes the result of \citep{Austrin_2022}.
A second defining feature of our contribution is the qualitative simplicity of our embedding theorem.
More precisely, to embed the hard instance $H$ in a random $G=(V,E) \samples \mathcal{G}(n,d)$, Austrin and Risse, first show a partition lemma that divides the vertices into two sets $T$ and $V \setminus T$, such that for any odd sized subset $U \subseteq T$, $V \setminus U$ has a perfect matching.

\highlight{Summarise the enormous work they have to go through.}
Contrast our results with that of \citep{Austrin_2022}, we show \emph{stronger} lower bounds for a \emph{larger} set of objects using \emph{simpler} proof techniques.


Our techniques are inspired by results from wide sense non blocking networks. \ari{Say something more about this.}
