...
\maketitle
\begin{abstract}
\highlight{\textbf{Attention Rajko:}} The introduction draft is very rough and I will clean this up.
...
In their seminal work \citep{cook1979relative}, Cook an Reckchow prescribed a program for separating $\CoNP$ from $\NP$ (which would also separate $\P$ from $\NP$).
...
If the size of $\Proof$ is $\SmallOmega{\poly(\Size{\PropFormula})}$, then $\CoNP \neq \NP$.
With no restrictions in place for how we describe a proof, we do not possess the tools to decide which of the above statements is true. 
However, it is widely believed that $\CoNP \neq \NP$, and thus, as an intermediate step, researchers have invested a significant amount of time proving size lower bounds more restricted proof systems. \ari{CITE SOME SEMINAL WORKS}.
When we say restricted proof system, we mean there is a fixed set of axioms and inference rules, and a restriction on the language used to for lines of the proof (\ari{see cite for more details}).
...
%another special case of Card(𝐺, 𝑡) is the so-called even coloring formula, introduced by Markstr ̈om [48], which is the case when 𝑡 = deg(𝑣)/2.
