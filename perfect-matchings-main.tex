...
\begin{figure}
  \caption{Proof sketch, \highlight{ADD in a sketch}}
	\label{fig:proof-outline}
\end{figure}

\begin{enumerate}
\item{First, we show (via Theorem \ref{thm:partition}) that we can partition the vertices of the graph $\Graph$ into two disjoint sets $\RedA$ and $\GreenB$, as shown in Figure \highlight{TODO}, of roughly equal size.
    ...
    Given this partitioning, we will topologically embed $H$ into the subgraph $G_{\GreenB}$ which is described by the edges where both endpoints lie in $\GreenB$. See Figure \highlight{TODO}.
  }
...
\]
  ...
  \highlight{Write an in summary block}
...
\begin{tcolorbox}
  Throughout the rest of this proof we describe  vertices in $\red{A}$ as \red{red} nodes, and vertices in $\green{B}$ as \green{green} nodes.
  For any set $X$, we use notation $\RedNodes{X} \Def X \cap A$ and $\GreenNodes{X} \Def X \cap B$ to denote the red and green nodes in $X$ respectively.
\end{tcolorbox}
...
The contribution lies in how we use the balanced partition to get a perfect matching in the residual graph in Theorem \ref{thm:partition}.
...
\emph{\highlight{Assume} towards a contradiction that $\BipartiteG$ is \textbf{NOT} non-blocking.
...
Also as $\Embedding{\Subdivision{H}{\sigma}}$ is a connected subgraph contained entirely in $B$, it must be entirely contained in one of the connected components connected to $\BadSet$. Figure \ref{fig:before-deletions} illustrates what we have described above pictorially.
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.5\textwidth]{assets/stock.png}
  \caption{Setup Before deletions}
