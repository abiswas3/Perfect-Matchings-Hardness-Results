...
In this work, we bridge this gap by showing refutations of perfect matchings in \emph{any} expander graph with a very mild spectral gap requires proofs with degree $\BigOmega{n/\log n}$.
...
We refer the reader to \citep{krajicek2019proof, ProofComplexityLecNotesPaul} for more details on these proof systems.
In this work, we focus on the algebraic and semi-algebraic proof systems of polynomial calculus (PC) and sum of squares (SoS) \citep{parrilo2000structured, boazCourse}.
In algebraic proof systems, instead of a propositional formula over $n$ variables, we are given a set $\Axioms =\{\axiom_i(\vec{x}) \text{ } |\text{ } i \in [m] \}$ of $m$ polynomial equations\footnote{Semi-algebraic proof systems also allow for inequalities but we will not deal with inequality constraints in this paper.} over $n$ variables $\vec{x} = \{x_1, \dots, x_n\}$.
In PC the coefficientcs can be over arbitrary fields $\Field$ and in the SoS the coefficients must be over the reals.
...
In PC and SoS, the proof $\Proof$ is itself expressed as a sequence of polynomials, and the size/complexity of the proof is measured by the degree of the proof polynomials that refutes $Q$ (see Section \ref{sec:proof-system-prelims} for more formal details).
We denote the minimum degree over all proofs that refute $\Axioms$ with $\Degree{Q \SOS \bot}$.
...
In this paper we study the complexity of refuting the \emph{perfect matching} principle in PC and SoS.
...
When $|V|$ is odd, we ask what is the complexity refuting $\PM{G}$ ?
With no further constraints on $G$, generic lower bounds for such refutations are not known.
In recent work, \citet{Austrin_2022} made progress  by showing that refuting the existence of perfect matchings in \emph{random $d$-regular graphs} in the Polynomial Calculus (PC) and Sum-of-Squares system requires proofs with degree $\BigOmega{\nicefrac{n}{\log n}}$ on average.
...
We start with a graph $H$ which known to be  difficult to refute.
We then show a embedding theorem that allows us to topologically embed a $H$ into \emph{any} expander graph with very mild spectral gap constraints ($\lambda \leq \epsilon d$), while still ensuring the parities of the all the path lengths remain the same.
...
\ari{Omitted, and re-written, but keeping your comment here for reference. We have not really emphasised our simplicity, and we have not contrasted our results with Killian and And Per. I know you say this how to *not* solve the problem, but the reviewers might not know this (or they might, I'm not sure). It is possibly worth thinking about. CS reviewers might not be as in tune with results as Math reviewers.}.
...
% \ari{I will tackle this on Friday. }
...
In Section \ref{sec:matching-machinery} and Section \ref{sec:embed-machinery}, we describe and prove the main lemmas needed by our topological embedding, and the proof for the residual graph having a perfect matching.
In Section \ref{sec:main-proof}, we provide the full proof of the hardness result, and in Section \ref{sec:related-work} we describe related results in proving proof complexity lower bounds using embedding techniques along with some open problems.
...
Given two sets $S, T \subseteq \Vertices{G}$, we  use $\CutEdges{S}{T}{E'}$ to denote the number of edges in $E'$ with one endpoint in $S$ and one endpoint in $T$. 
...
Next, we give a definition for pseudorandom graphs.
...
\begin{lemma}[$f$-Factor Criterion]\label{lemma:tutte-criterion-factor}
...
\begin{lemma}[$f$-Factor Lemma]\label{lemma:f-factor}
...
\emph{Assume towards a contradiction} that $\BipartiteG$ is \textbf{not} non-blocking.
...
\eqref{eq:assumption} comes from our \emph{assumption} on the size of $T$.
\eqref{eq:assumption-lambda} comes from our assumption that $\lambda \leq  d/c_2$ and $|S| \leq \frac{n\lambda}{c_1 d}$.
\eqref{eq:constants} contradicts the balanced sets property in Equation \eqref{eq:balanced}, so our \emph{assumption} must have been wrong.
...
\item{ As $n$ is odd, and $\Size{\Vertices{G_{\EmbeddingFunc}}}$ is odd, we have that $U = \Vertices{G} \setminus \Vertices{G_{\EmbeddingFunc}}$ has even size.
    The balanced partitioning gives us $\MinimalDegree{G[U]} \geq \frac{11d}{16}$, and as $\lambda < d/50$, we can invoke the \nameref{thm:perfect-matching} to show $G[U]$ has a perfect matching $M$.
...
As $\lambda < \epsilon d$, we invoke the \nameref{lemma:f-factor} to show that $G'$ has a $t-1$ factor.
