...
However, it is widely believed that $\CoNP \neq \NP$, and thus, as an intermediate step, over the last 50 years researchers have invested a significant amount of time and energy in proving size lower bounds for more restricted proof systems. \ari{CITE SOME SEMINAL WORKS here}.
...
A simple example of a language for describing proofs is that of truth tables i.e. given a formula $\PropFormula$, a valid proof is simply the truth table for the formula.
...
However, the above proof system is extremely restricted, and to make progress towards the main goal the research community has considered increasingly more expressive proof systems \ari{cite many works here}.
...
In PC and SOS, the proof $\Proof$ is itself expressed as polynomial (see Section \ref{sec:proof-system-prelims} for more details). 
The size of a proof in the above systems is measured by the maximum of degree of the proof polynomial that refutes $Q$.
We denote the degree of such polynomials with $\Degree{Q \PC \bot}$ and $\Degree{Q \SOS \bot}$ for PC and SOS respectively, and the main goal is to study how the complexity of the degree of refutals for a $Q$.
