...
\begin{theorem}[Embedding Theorem]\label{theorem:embedding-theorem}
  Let $G=(V,E)$ denote an $\EnDeeLambda$ graph with $\lambda \le \epsilonL d$, and $H$ be an arbitrary sparse graph with at most $t = \lfloor \frac{ n}{20(\MaxDegree{H}+2)\log_D \na}\rfloor $ vertices with max degree $\MaxDegree{H} = \BigO{1} < d$, where $D=d/\lambda$.
  For any subset $\GreenB \subseteq V$ with $\MinimalDegree{G[\GreenB]} \ge \frac{3d}{16}$, $G[\GreenB]$ contains $H$ as a topological minor $\Subdivision{H}{\sigma}$ where every edge in $H$ is an odd sized path in $\Subdivision{H}{\sigma}$.
\end{theorem}
\begin{proof}
...
\begin{itemize}
\item Define $E' = \emptyset$ to begin with, and as $\BipartiteG$ is a $(D, n/20)$-non blocking bipartite graph  $E'$ is a safe state.
...
Notice that  $|M| \leq 2tk < n/20$, so $M$ is a safe state.
...
They key thing to note is that $\GlobalEdgeSet < n/20$.
To see why, $H$ has at most $t\MaxDegree{H}/2$ edges.
When these edges are mapped to paths of length $2k+1$, the total number of edges in $\GlobalEdgeSet$ is $\frac{t\MaxDegree{H}}{2}(2k+1) < t/2\MaxDegree{H}3k = n/20$.
...
\item{Invoke Theorem \ref{thm:partition} with parameters $c=3/4$ and $\epsilon = 1/16$ to get a partition of $V = \RedA \cup \GreenB$ such that for every $u \in V$: $\frac{11}{16}d  \leq   \Size{\Neighbourhood{E}{u} \cap \RedA} \leq \frac{13}{16}d$ and $\frac{3}{16}d  \leq   \Size{\Neighbourhood{E}{u} \cap \GreenB} \leq \frac{5}{16}d$.}
    ...
    \highlight{In the embedding lemma include the max degree of $H$ as a parameter.
  }
