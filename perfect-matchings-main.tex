...
\begin{proof}
Let $\epsilon, A$ and $B$ be as defined in Lemma \ref{lemma:partition} (Figure \ref{subfig:partitionA}).
Define $\tilde{d} = 1 - (2\epsilon d)$ and let $\tilde{G}=(\tilde{V}, \tilde{E})$ be short hand for $\Graph[V \setminus \Embedding{\Subdivision{H}{\sigma}}]$.
...
Let $\tilde{n} = |\tilde{V}|$.
Pick arbitrary $U \subset \tilde{V}$ such that $|U| \leq \tilde{n}/2$.
Then we have 

\begin{align}
\CutEdges{U}{\Complement{U}}{\tilde{E}} &\geq 	\frac{d}{n}u(n-u) - \lambda\sqrt{u(n-u)(1 - u/n)(1- (n-u)/n)} \label{eq:step1}\\
...
\begin{align}
	\CutEdges{U}{\Complement{U}}{\tilde{E}} &\geq 	\CutEdges{U}{\Complement{U}}{\tilde{E}} - \tilde{d}u \\
&\geq \frac{u}{2}(d - \ExpansionFactor{G}) - \tilde{d}u \\
&= \frac{u}{2}(d - \tilde{d} - \ExpansionFactor{G}) \label{eq:lambda-assumption} \\
&\geq \tilde{d}
\end{align}


\eqref{eq:lambda-assumption} comes from the assumption that $\ExpansionFactor{G} \leq d - \tilde{d}  -2$.
