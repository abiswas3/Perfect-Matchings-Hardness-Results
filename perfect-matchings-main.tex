...
\ari{Change this to just directly the general proof.}
...
\ari{Change the cut edges notation third parameter to include $G$ instead of $E$.}
...
Thus, we conclude $\PM{G}|_\rho \equiv \PM{H}$. Therefore by Lemma \ref{lemma:affine_restriction} above, we get that $\Card{G, \vec{t}} \in \BigOmega{\frac{n}{\log n}}$. This concludes the proof for Theorem \ref{thm:main-thm}.
}
  
\item{If we want to show our hardness by reducing the complexity of refuting $\Card{G, \vec{t}}$ to the complexity of refuting $\PM{H}$.
...
As $\MaxDegree{H} \leq 5$, every vertex $u \in G$ loses at most $C=5 + 1$ edges in this process.
  Thus we have $\MinimalDegree{G'} = d - C$. Setting $\lambda < \epsilon d < d/50$ with $\epsilon < 1/100C^{3/2}$, we invoke the \nameref{lemma:f-factor} to show that $G'$ has a $t-1$ factor.
Now extend $\rho$ by the following process - for every $u \in G'$, set edges $e$ that are included in the $t-1$ factor to $\rho(e) = 1$.
Observe that for $G$ to satisfy $\Card{G, \vec{t}}$, we need $G_{\EmbeddingFunc}$ to satisfy a perfect matching.
Thus refuting $\Card{G, \vec{t}}$ is reduced to refuting $\PM{H}$.
}
