...
\ari{Omitted, and re-written, but keeping your comment here for reference. We have not really emphasised our simplicity, and we have not contrasted our results with Killiand And Per. I know you say this how to *not* solve the problem, but the reviewers might not know this (or they might, I'm not sure). It is possibly worth thinking about. CS reviewers might not be as in tune with results as math reviewers.}.
...
\item{ As $n$ is odd, and $\Size{\Vertices{G_{\EmbeddingFunc}}}$ is odd, we have that $U = \Vertices{G} \setminus \Vertices{G_{\EmbeddingFunc}}$ has even size.
    The balanced partitioning gives us $\MinimalDegree{G[U]} \geq \frac{11d}{16}$, and as $\lambda < d/50$, we can inoke the \nameref{thm:perfect-matching} to show $G[U]$ has a perfect matching $M$.
...
Thus, we conclude $\PM{G}|_\rho \equiv \PM{G_{\EmbeddingFunc}} \equiv \PM{H}$. Therefore by Lemma \ref{lemma:affine_restriction} above, we get that $\Card{G, \vec{t}} \in \BigOmega{\frac{n}{\log n}}$. If $t=1$, then this concludes the proof for Theorem \ref{thm:main-thm}.
}
