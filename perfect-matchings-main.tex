...
When we say restricted proof system, we mean there is a fixed set of axioms and inference rules, and a restriction on the language that can be used to write down lines of the proof. We refer the reader to \cite{ProofComplexityLecNotesPaul} for detailed introduction to proof systems.
A simple example of a language for describing proofs is that of truth tables i.e. given a formula $\PropFormula$, a valid proof is truth table for the formula.
...
However such a proof system is extremely restricted, and to make progress towards the main goal the research community has considered more expressive proof systems \ari{cite many works here}.
Figure \ref{fig:example-proof-systems} lists some these proof systems and describes the relationships between them.
Proof systems listed at the top of Figure \ref{fig:example-proof-systems} are more expressive\footnote{Expressiveness is formalised by the notion of $p$-domination, and we refer the reader to \citep[Definition 1.6]{ProofComplexityLecNotes} for more details.}, and lower bounds for such proof systems are subsume claims about the complexity of proofs lower in the figure.\par
In this work, we focus on the semi-algebraic proof systems of polynomial calculus (PC) and sum of squares (SOS).
