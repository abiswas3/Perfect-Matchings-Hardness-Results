...
In their seminal work \citep{cook1979relative}, Cook an Reckchow prescribed a program for separating $\CoNP$ from $\NP$.
Given a propositional formula $\PropFormula$, the task is to come up with a proof $\Proof$ of the claim that $\PropFormula$ is unsatisfiable.
The holy grail of computer science and mathematics is to prove that the size any such proof $\Proof$ is $\SmallOmega{\poly(\Size{\PropFormula})}$, and thus we can conclude that $\CoNP \neq \NP$.
With no restrictions in place for the language of how we describe a proof, we are currently no close to proving the above statement. 
However, it is widely believed that $\CoNP \neq \NP$, and thus, as an intermediate step, over the last 5- years researchers have invested a significant amount of time and energy in proving size lower bounds for more restricted proof systems. \ari{CITE SOME SEMINAL WORKS here}.\par
When we say restricted proof system, we mean there is a fixed set of axioms and inference rules, and a restriction on the language that can be used to write down lines of the proof. We refer the reader to \cite{ProofComplexityLecNotesPaul}

A simple example of proof system is that of truth tables i.e. given a formula $\PropFormula$, a valid proof is truth table for the formula.
For such a proof system, for a formula on $n$ variables, the size of the proof is $2^n$ and it is easy to see that we cannot use shorter proofs.
However such a proof system is extremely restricted, and to make progress towards the main goal the research community has considered more expressive proof systems \ari{CITE}.
Figure \ref{fig:example-proof-systems} lists these systems and describes the relationships between them.
Proof systems listed at the top of Figure \ref{fig:example-proof-systems} are more expressive\footnote{Expressiveness is formalised by the notion of $p$-domination, and we refer the reader to the \ari{CITE} for more details.}, and lower bounds for such proof systems are stronger results.\par
Central to this work are proofs that serve as refutations in semi-algebraic proof systems like polynomial calculus (PC) and sum of squares (SOS).
...
In PC, the polynomials can be over \emph{any} field $\Field$, and in SOS, the polynomials are over the reals.
We say a proof $\pi$ refutes $Q$, if it is a proof of the claim (in the specified language) that there exists no assignment of $\vec{x} \in \Field$ that satisfies \emph{all} the polynomial equations in $Q$. 
PC and SOS proofs are themselves expressed in the language of polynomial equations (see Section \ref{sec:proof-system-prelims} for more details). 
Thus, the size of proofs in the above systems is measured by the maximum of degree of the proof polynomial that refutes $Q$.
We denote the degree of such polynomials with $\Degree{Q \PC \bot}$ and $\Degree{Q \SOS \bot}$ for PC and SOS respectively.
These proof systems are quite powerful in that it is often possible to express properties of combinatorial objects like an unweighted undirected graph by a system of polynomial equations.
Given an undirected graph $G=(V,E)$, we define variables $x_1, \dots, x_{|E|}$.
For every $e \in E$, if the polynomial equation $x_e(1 - x_e) = 0$ were to be satisfied, then it restricts the value of the variables to bits i.e. $x_e \in \bit$ for all $e \in E$.
Let $\vec{b} = (b_1, \dots, b_{|V|})  \in \Integers^{|V|}$ be a constant vector.
Then, for each $v \in V$, define the polynomials $f_v \Def \sum_{e \in E} x_e - b_v = 0$.
We denote $\Card{G, \vec{b}}$ as above system of linear equations along with the Boolean constraints.
More formally,

\begin{definition}[Card]\label{def:card}Given a graph $G=(V,E)$, and a vector of constants $\vec{b} = (b_1, \dots, b_{|V|}) \in \Field^{|V|}$, $\Card{G, \vec{b}}$ describes the following system of equations over variables $x_1, \dots, x_{|E|} \in \Field^{|E|}$ 
...
Note if there was an assignment of variables in $\vec{x} \in \bit^{|E|}$ that satisfied all the equations in $\Card{G, 1^{|V|}}$, it would imply that the graph $G$ has a perfect matching.  
When $|V|$ is odd, we know that any graph $G$ cannot have a perfect matching.
In other words, $\Card{G, 1^{|V|}}$ cannot be satisfied.
Thus, we study the complexity of PC and SOS refutations for $\Card{G, 1^{|V|}}$  when $|V|$ is odd.
...
\citet{Austrin_2022} made partial progress on the above problem by showing that the perfect matching principle requires large size on \highlight{random $d$-regular graphs} (for some constant $d$) in the Sum-of-Squares, and Polynomial Calculus.
More formally they show that 

\begin{theorem}{\citep{Austrin_2022}}{thm:prev-thm}
There is a constant $d_0 \in \Naturals$ such that for all $d \geq d_0$, and $b \in [d]$, the following holds asymptotically almost surely over a random $d$-regular graph $G$ on $n$ vertices:
\begin{enumerate}
    \item{ $\Degree{\Card{G, \vec{b}} \PC \bot} = \BigOmega{\frac{n}{\log n}}$} 
    \item{$\Degree{\Card{G, \vec{b}} \SOS \bot} = \BigOmega{\frac{n}{\log n}}$}
\end{enumerate}

\AttentionRajko{This theorem is copied from the Risse paper. $b$ is given as a scalar but then used in vector form. Are all the coordinates the same or different?}

\end{theorem}


\paragraph{Our Contribution} In this work, we directly answer Buss and Nordstr{\"o}m's question, and show that refutations for $\Card{G, \vec{b}}$ have large size for Sum-of-Squares and Polynomial Calculus for expander graphs.
...
Our result is inspired by the general technique used in \citep{Austrin_2022}.
Given a random $d$-regular graph $G$ on $n$ vertices, they start with a worst case hard graph $H$ constructed by \citet{buss1999linear}.
Such a graph is known to have $\Degree{\PerfectMatching{G} \PC \bot} = \BigOmega{\Size{V(H)}}$ (see \citep[Appendix A]{Austrin_2022} for details about how $H$ is constructed).
...
%another special case of Card(��, ��) is the so-called even coloring formula, introduced by Markstr ̈om [48], which is the case when �� = deg(��)/2.
