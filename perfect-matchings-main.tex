...
However, it is widely believed that $\CoNP \neq \NP$, and thus, as an intermediate step, over the last 5- years researchers have invested a significant amount of time and energy in proving size lower bounds for more restricted proof systems. \ari{CITE SOME SEMINAL WORKS here}.
...
In a semi-algebraic proof system like PC and SOS, instead of a propositional formula over $n$ variables, we are given a set $Q=\{q_i(\vec{x}) \text{ } |\text{ } i \in [m] \}$ of polynomial equations over $n$ variables $\vec{x} = (x_1, \dots, x_n) \in \Field^n$ as the problem instance.
...
In PC and SOS, the proof $\Proof$ itself expressed as polynomial (see Section \ref{sec:proof-system-prelims} for more details). 
The size of a proof in the above systems is measured by the maximum of degree of the proof polynomial that refutes $Q$.
We denote the degree of such polynomials with $\Degree{Q \PC \bot}$ and $\Degree{Q \SOS \bot}$ for PC and SOS respectively, and the goal to prove lower bounds on the degree of this proofs.
Often one can use semi-algebraic proof systems to describe properties of combinatorial objects like graphs.
More specifically, given an undirected graph $G=(V,E)$, and a vector $\vec{b} = (b_1, \dots, b_{|V|})  \in \Field^{\Size{V}}$, we define variables $x_1, \dots, x_{|E|} \in \Field^{\Size{E}}$ and system of polynomial constraints $Q = \Card{G, \vec{b}}$ as the following.
...
The main task then is to embed $H$ into $G$ in such a way that that the hardness of $H$ is transferred to $G$.
