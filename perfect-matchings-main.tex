...
Suppose $G$ is an $\EnDeeLambda$ graph with $\lambda < \epsilon d$, where $\epsilon < 1/100C^{3/2}$ for some universal constant $C > 0$ and let $d \ge d(C_0)$.
...
\begin{proof}
  \highlight{This is not really right}
  Without loss of generality $t \le d/2$ (otherwise we can flip the 0's and 1's and just show the result for $\Card{G, \vec{d-t}}$).
  Using the \nameref{thm:partition} lemma, we partition $G$ into two sets $\RedA$ and $\GreenB$, such that $\MinimalDegree{G[\RedA]}\ge d-C$.
  We embed $H$ into $G[\GreenB]$ using the \nameref{theorem:embedding-theorem}.
  This allows us to invoke Lemma \ref{lemma:f-factor} to show that $G[\RedA]$ contains $t-1$ factor.
  For every $e \in \Edges{G[\RedA]}$, set $x_e = 1$.
  Now refuting $\Card{G, \vec{t}}$ reduces to showing that $G[\GreenB]$ cannot have a perfect matching.
\end{proof}
