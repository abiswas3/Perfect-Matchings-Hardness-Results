...
With respect to $\GlobalEdgeSet \cup \Big(\cup_{i=1}^{j} (M_i \cup M'_i)\Big)$, add $\Size{M_j}$ safe neighbours to $S_{j+1}$ where $\Size{M_j} \le \min\{t, |S_{z-1}|D\}$.
Add to $M_j$ the safe edges that connect $S_{j-1}$ to $S_j$.
    ...
    Thus, we can always find at least one safe neighbour for each $x \in S_{j-1}$.
