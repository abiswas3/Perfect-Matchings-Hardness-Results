...
\begin{proof}
  ...
  Given $G$ is an $\EnDeeLambda$ graph and $\lambda \leq \epsilon d < d-2$, we have \highlight{Benny/Michal survey} that $\OddComponents{S \cup T} \le \Size{S \cup T} = |S| + |T|$.
...
\textbf{Case 2b}: $ \frac{\Size{S}}{C+3} < \Size{T} < \Size{S}$
...
\begin{proof}
  ...
  Now consider the subgraphs constructed by partitioning the edges of $G$ into $G_0$ and $G'$, where $G'$ is constructed by deleting $C=d/2$ edges from every vertex in $G$.
  Thus, $G'$ is $d - C$ regular, and $G'$ is $C$ regular.
