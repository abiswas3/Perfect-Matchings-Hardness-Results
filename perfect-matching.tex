...
As this applies broadly across proof systems, we restrict our preliminaries to PC and SoS for brevity.} of \emph{polynomial calculus (PC)} and \emph{sum of squares (SoS)}. %\citep{boazCourse,parrilo2000structured}.
...
\ari{Check how to use notation.}
\begin{lemma}[Perfect Matching Lemma]\label{thm:perfect-matching}
Given a constant $C \ge 0$, and define $d_0 = d_0(C)$ such that $d_0(0.95 - 0.9) > C$. 
For all  $(n, d, \lambda)$-graphs $G$ with $d > d_0$ and $\lambda < d/50$  the following holds.
For any $U \subseteq V(G)$, with $\delta(G[U]) \ge 0.95d$, let $H$ be a subgraph obtained by deleting at most $C$ edges incident on every vertex of $G[U]$. 
...
\begin{proof}
  ...
  Given $C \ge 0$, we can make $d_0 := d_0(C)$ large enough such that such that $\delta(H) \ge \cTilde$.
...
Now as all $C_i$'s are disjoint connected components, there can be no edge between $X^\star$ and $Y^\star$. Therefore, we have found a partition $X^\star \cup Y^\star = U \setminus S$ with $|X^\star|, |Y^\star| \ge |S|/3$ without an edge between them, which contradicts our assumption. 
...
Then by a simple counting argument
\begin{equation}
...
\end{equation}
  ...
  We will show that
    ...
    \end{equation}
  We were given that $\delta(G[U]) \ge 0.95d$, as $d$ is large enough, we have that $0.95d|X| - 0.9d|X| > C|X|$ edges must have one endpoint in $X$ and the other in $U \setminus (X \cup S) = Y$.
Now as we construct $H$ by deleting at most $C$ edges incident on each vertex of $U$.
So even if we removed all $C|X|$ of the edges described above, we will still have at least one edge with end point in $X$ and the other endpoint in $Y$. 
...
For any \textit{even} sized subset $U \subseteq V(G)$ with subgraph $H \subseteq G[U]$, if $\delta(H) \ge \cTilde$, then $H$ satisfies the \nameref{lemma:tutte-criterion}, and therefore $H$ contains a perfect matching.  
\end{corollary}
...
Therefore the conditions of Lemma \ref{thm:perfect-matching} are satisfied \rajko{No! That lemma considers induced subgraphs, whereas here we consider graphs obtained by removing edges. I suggest changing the statement of that lemma to apply to subgraphs of induced graphs. Then it will be sufficient for both applications. Most likely nothing needs to be changed in the proof, but please double check.}, from which we can conclude that $$\OddComponents{G' \setminus (S \cup T)} \le \Size{S \cup T} = |S| + |T|$$
Thus, to prove \eqref{eq:main-goal}, it suffices to show \rajko{First line: shouldn't it be $\Gamma_{G'}(w)$?} \rajko{The second line is not equality, as we only have lower bound on the minimum degree.}
  \begin{align}
