...
\rajko{Add somewhere for me: ``Research supported by the Marsden Fund of the Royal Society of New Zealand.''}

\rajko{Please make Lemma, Corollary, and Theorem to be of the same colour as Lemma. I find Corollary yellow distracting, and Theorem is too dark.}
\end{abstract}
...
\begin{enumerate}
	\item From polynomials $f=0$ and $g=0$ where $f,g \in \Field[\vec{X}]$ we can derive $\alpha f + \beta g = 0$ for $\alpha, \beta \in \Field$. \rajko{You mean polynomial equations, not polynomials?}
...
We make use of the following two well known criteria of Tutte \cite{tutte1952factors, tutte1947factorization}. Note that both of the lemmata ask for properties which are stronger than what Tutte criteria requires; however, they are easier to state and verify in our application. We denote with $q(G)$ the number of connected components in a graph $G$.
...
If a graph $G$ has  \emph{even} number of vertices and for every $S \subseteq V(G)$ we have $\OddComponents{G[V(G) \setminus S]} \leq |S|$, then $G$ contains a perfect matching.
\end{lemma}
...
\begin{lemma}[Tutte's Generalised Criterion]\label{lemma:tutte-criterion-factor}
  ...
  \[ |S|f \geq \OddComponents{G[S \cup T]} + \sum_{w \in T} (f - \Size{\Neighbourhood{G}{w} \setminus S}). \]
...
For every $0 < c < 1$ and $\epsilon > 0$, there exists $d_0$ such that the following holds. If $G$ is a $d$-regular graph, for some $d \ge d_0$, then there exists a subset $A \subseteq V(G)$ such that
$$
    cd - \epsilon d \leq \left| \Neighbourhood{G}{v} \cap A \right| \leq cd + \epsilon d 
$$
for every $v \in V(G)$.
...
Suppose $G$ is an $(n, d, \lambda)$-graph, where $\lambda < d/50$. If $U \subseteq V(G)$ is of even size and $\delta(G[U]) \ge \cTilde$, then $G[U]$ satisfies the property of \nameref{lemma:tutte-criterion}. Consequently, $G[U]$ contains a perfect matching. 
\end{lemma}
...
Thus, we can assume $|S| < |U|/2$.
 We claim the following:  \rajko{Rephrased}
...
\rajko{Rephrased the statement of the next lemma.}
...
\begin{proof}
  ...
  We need to show that for any pair of disjoint sets $S, T \subseteq \Vertices{G'}$, we have \rajko{$q(S \cup T)$ should be $q(G'[S \cup T])$. Change throughout the proof.}
  \begin{align}
\OddComponents{S \cup T} &\le \Size{S}f - \sum_{w \in T} (f - \Size{\Neighbourhood{G'}{w} \setminus S}).
  \end{align}

As $G$ is an $\EnDeeLambda$ graph with $\lambda < \epsilon d < d/50$ and $G'$ has minimal degree $\MinimalDegree{G'} \ge d - C >  11d/16$, by the Lemma above \rajko{Properly reference the lemma by its number}, $G$' satisfies the  \nameref{lemma:tutte-criterion}, \rajko{This is not what Tutte's criterion states, so explain it} that is $\OddComponents{S \cup T} \le \Size{S \cup T} = |S| + |T|$.
...
\rajko{You need some text between these two align enviroments. Explain what is it that you want to show.}
...
\item{Let $B = V(G) \setminus A$. \rajko{$B$ should be defined earlier.} From the partition lemma above \rajko{Not the partition lemma itself, but (25) and (26)}, $\Size{B} > n/20$, with room to spare and $\Size{E(G[B])} \ge nd/20$. $G[B]$ \rajko{Why is $G[B]$ a sentence on its own?}.
...
\item{ As $n$ is odd and $\Size{\Vertices{G_{\EmbeddingFunc}}}$ is odd, we have that $U = \Vertices{G} \setminus \Vertices{G_{\EmbeddingFunc}}$ has even size. From \eqref{eq:size_nbr_A} we have that $G[U]$ has minimum degree at least $\cTilde$. As $\lambda < d/50$, we can invoke the \nameref{thm:perfect-matching} to conclude $G[U]$ has a perfect matching $M$. 
}
  
\item{
  Consider the subgraph $G' \subseteq G$ obtained by deleting all edges $e \in \Edges{G_{\EmbeddingFunc}} \cup M$, where $M$ is the perfect matching from the step above. As $\MaxDegree{H} \leq 5$, every vertex $u \in G$ loses at most $5$ edges in this process. Thus, we have $\MinimalDegree{G'} \ge d - 5$. As $\lambda < \epsilon d$, by \nameref{lemma:f-factor} we have that $G$ contains a $(t-1)$-regular spanning subgraph $G'$. \rajko{The last sentence is wrong. You want to say that $G'$ contains a $(t-1)$-regular spanning subgraph. Consequently, the definition of $\rho$ is not correct.}}
