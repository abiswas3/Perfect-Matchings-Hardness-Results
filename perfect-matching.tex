...
Given a constant $C \ge 0$, define $d_0 = d_0(C)$ such that $d_0(0.95 - 0.9) > C$. 
For all  $(n, d, \lambda)$-graphs $G$ with $d > d_0$ and $\lambda < d/50$  the following holds.
For any $U \subseteq V(G)$, with $\delta(G[U]) \ge 0.95d$, let $H$ be a subgraph obtained by deleting at most $C$ incidental edges  per vertex of $G[U]$. 
Then for all $S \subseteq U$, $H\setminus S$ has at most $|S|$ connected components, that is, $q(H \setminus S) \le |S|$.
\end{lemma}

\begin{proof}
  Let $U \subseteq V(G)$ with $\delta(G[U]) \ge 0.95d$ and  let $H \subseteq G[U]$ be the subgraph obtained by deleting at most $C$ edges incident on any vertex of $G[U]$. 
  Consider some $S \subseteq U$.
  We aim to show that the graph $H \setminus S$ has at most $|S|$ connected components.
  If $|S| \ge |U|/2$ then as $H \setminus S$ has fewer than $|S|$ vertices, so the the upper bound on connected components trivially holds. \\

\flushleft
Thus, for the remainder of the proof we can safely assume $|S| < |U|/2$.
 We claim the following:

% For every partition $X \cup Y = U \setminus S$, with $|X|,|Y| \ge |S|/3$,
%  \begin{equation}
%   \text{If }\CutEdges{X}{Y}{H} \ge 1 \implies  q(H \setminus S) \le |S|. \label{eq:mini-claim}
%  \end{equation}
%
\begin{equation}
  \text{For every partition } X \cup Y = U \setminus S,\text{ with } |X|,|Y| \ge |S|/3,
  \quad
  \CutEdges{X}{Y}{H} \ge 1 \implies  q(H \setminus S) \le |S|.
\label{eq:mini-claim}
\end{equation}
\flushleft
To see why, assume towards a contradiction that there exists an edge between \emph{every} partition $X \cup Y = U \setminus S$, where $|X|, |Y| \ge \Size{S}/3$, \emph{and} $H \setminus S$ has more than $|S|$ connected components. Denote the  vertex sets of these components by $C_1, \ldots, C_k$, for some $k > |S|$.
...
To complete our proof, it suffices to show $e_H(X, Y) \geq 1$.\\
\flushleft
...
Therefore the conditions of Lemma \ref{thm:perfect-matching} are satisfied with $G'$ and $G$ playing the role $H$ and $G[U]$ respectively, in the lemma statement.
...
From this we can conclude that $$\OddComponents{G' \setminus (S \cup T)} \le \Size{S \cup T} = |S| + |T|$$
...
% \rajko{First line: shouldn't it be $\Gamma_{G'}(w)$?} \rajko{The second line is not equality, as we only have lower bound on the minimum degree.}
  \begin{align}
    |S| + |T| &\le |S|f - \sum_{w \in T} (f - \Size{\Neighbourhood{G'}{w} \setminus S})   \\
              &\le \Size{S}f - \Size{T}f + \Size{T}(d-C) - \CutEdges{S}{T}{G'}  \label{eq:to-show}
  \end{align}  
