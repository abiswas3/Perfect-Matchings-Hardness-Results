...
\ari{Why not mention the Buss and Norstrom thing?}
...
\begin{definition}[Sum Of Squares Refutation]\label{def:sum-of-squares} Given a set of $m$ polynomial equality constraints $\Axioms$ over the reals, a Sum of Squares (SoS) refutation is a set of polynomials $\Proof = (t_1, \dots, t_m; s_1, \dots, s_a)$ such that
...
Note that both of the lemmata ask for properties which are stronger than what Tutte criteria requires\footnote{More specifically, in the Tutte criterion $q(G \setminus S)$ denotes the number of \emph{odd} sized connected components in the graph $G \setminus S$.}; however, they are easier to state and verify in our application. We denote with $q(G)$ the number of connected components in a graph $G$.
...
\begin{lemma}[Tutte's Generalised Criterion]\label{lemma:tutte-criterion-factor}
  ...
  \[ |S|f \geq \OddComponents{G \setminus (S \cup T)} + \sum_{w \in T} (f - \Size{\Neighbourhood{G}{w} \setminus S}). \]
...
Then the induced sub-graph $G[B]$ contains $H^{\sigma}$ and $\sigma(e)$ is odd for every $e \in E(H)$.
\end{corollary}
...
As $G[B]$ has at least $Cn$ edges (by \nameref{lemma:expanders-mixing-lemma}), we satisfy all the constraints to invoke the \nameref{thm:embedding_top} to conclude that $G[B]$ contains $H^\sigma$.
Note that $\sigma(e) \le 2 + \log n$, and we know that $H$ has at most $\alpha \frac{n}{\log n}$ vertices. 
...
For any $U \subseteq V(G)$, if $\delta(G[U]) \ge \cTilde$, then for all $S \subseteq U$, $G[U \setminus S]$ has at most $|S|$ connected components. 
\end{lemma}

\begin{proof}
  Fix arbitrary $U \subseteq V(G)$, and let $\delta(G[U]) \ge \cTilde$.
