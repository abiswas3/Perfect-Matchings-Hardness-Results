...
\begin{corollary} \label{cor:embedding_top}
  ...
  Suppose $G$ is an $(n,d,\lambda)$-graph where $d \ge d_0$ is a constant \rajko{We don't need that $d$ is constant.} and $\lambda < \epsilon d$, and $n \ge n_0$. 
...
In the last step we used the fact that $d$ is a constant \rajko{Where did you use that $d$ is constant?}, and that $\lambda < \epsilon d$.  \rajko{Please fix this sentence:} This and the lower bounds on $S,T$ and $B$, we can make $\epsilon$ sufficiently small with respect to $\xi$ to get our upper bound.
...
As $G[B]$ has at least $Cn$ edges (by \nameref{lemma:expanders-mixing-lemma}), $\sigma(e) \le 2 + \log n$, and $H$ has at most $\alpha \frac{n}{\log n}$ vertices, we can invoke \nameref{thm:embedding_top} to conclude that $G[B]$ contains $H^\sigma$.
\end{proof}
...
If $G' \subseteq G$ has minimum degree $\MinimalDegree{G'} \ge d - C > \frac{9d}{10}$ \rajko{If we are only using $\delta_{G'} > 9d/10$, then there is not need to mention $C$ at all in the statement.}, then $G'$ contains a spanning $f$-regular subgraph for any even $2 \le f \le d/2$.
\end{lemma}
