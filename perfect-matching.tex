...
In the lemma below, the parameter $C$ should be read as the subgraph $G'$ obtained by deleting a constant number of edges from the $\EnDeeLambda$ graph $G$.
...
\begin{proof}
  ...
  As $\epsilon < \nicefrac{1}{100C^{3/2}}$ and $C > 1$, we have that $\epsilon < 1/50$.
This implies that $G$ is an $\EnDeeLambda$ graph with $\lambda < d/50$.
Additionally, we also assume that $G'$ has minimal degree $\MinimalDegree{G'} \ge  9d/10$, therefore the conditions of Lemma \ref{thm:perfect-matching} are satisfied, from which we can concluce that $$\OddComponents{G' \setminus (S \cup T)} \le \Size{S \cup T} = |S| + |T|$$
