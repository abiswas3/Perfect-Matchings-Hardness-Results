...
In PC, the coefficientcs can be over an arbitrary, but fixed field $\Field$ and in the SoS the coefficients are  over the reals.
...
Two common measures of the complexity of a semi-algebraic proof are the length of the sequence and the largest degree of the proof polynomials that refute $Q$ (see Section \ref{sec:proof-system-prelims} for more formal details).
...
The main technical ingredient of Austrin and Risse is also a 	similar embedding theorem, but one that critically relies on the underlying graph being random.
...
In the lemma below, the parameter $C$ should be read as the subgraph $G'$ obtained by deleting $C$ edges from the $\EnDeeLambda$ graph $G$. \rajko{We delete at most $C$ edges incident to each vertex.}
...
If $G' \subseteq G$ has minimum degree $\MinimalDegree{G'} \ge \frac{9d}{10}$, then $G'$ contains a spanning $f$-regular subgraph for any even $2 \le f \le d/2$. \rajko{This is rather confusing statement. Why not just say $\MinimalDegree{G'} \ge d - C$? In the proof we can then use that this implies $\delta_{G'} \ge 9d/10$, but for the statement that is not important.} 
\end{lemma}
...
\begin{proof}
  ...
  As $\epsilon < \nicefrac{1}{100C^{3/2}}$ and $C > 1$, we have that $\epsilon < 1/50$ \rajko{Even better, we have $\epsilon < 1/100$ :)}.
This implies that $G$ is an $\EnDeeLambda$ graph with $\lambda < d/50$.
Additionally, we also assume that $G'$ has minimal degree $\MinimalDegree{G'} \ge  9d/10$ \rajko{Let us mention that this follows from $\delta_{G'} \ge \delta_G - C$ and $d \ge d_0(C)$}, therefore the conditions of Lemma \ref{thm:perfect-matching} are satisfied, from which we can conclude that $$\OddComponents{G' \setminus (S \cup T)} \le \Size{S \cup T} = |S| + |T|$$
