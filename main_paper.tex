...
\begin{lemma}[Non-blocking]\label{lemma:bipartitie-is-non-blocking}	
\end{lemma}
...
\begin{lemma}[Perfect Matching On The Outside]\label{lemma:perfect-matching}
Given $\EnDeeLambda$ graph $\Graph$.


 let $A$ and $B$ be the partitioning defined in Lemma \ref{lemma:partition}.
	
\end{lemma}

\begin{proof}
Define $\tilde{d} = 1 - (\epsilon d + \sqrt{\nicefrac{d}{2}\log (2/\delta)})$
\ari{Define things clearly.}
Let $\tilde{G} = \Graph[V \setminus \Embedding{\Subdivision{H}{\sigma}}]$. 
...
By lemma \ref{lemma:partition}, we know that the minimal degree of $\MinimalDegree{\tilde{\Graph}} \geq \tilde{d}$.
