...
\begin{corollary} \label{cor:embedding_top}
  For every $D \in \mathbb{N}$ there exist $d_0 \in \Naturals, \epsilon \in (0,1)$, such that the following holds. Suppose $G$ is an $(n,d,\lambda)$-graph where $d \ge d_0$ and $\lambda < \epsilon d$. Let $B \subseteq V(G)$ be a subset of size $|B| \ge n/10$.
  If $e(G[B]) > Cn$ for some $C >0$, then the induced subgraph $G[B]$ contains $H^{\sigma}$, where $H$ is any graph with maximum degree at most $D$ and at most $\alpha n / (11 \log n)$ vertices, and $\sigma(e)$ is odd for every $e \in E(H)$.
\end{corollary}
\begin{proof}
...
Dividing both sides with $b(b-1)$, and using the fact that $\frac{b}{b-1} = 1 + \frac{1}{b-1}$, we get
...
Therefore, for every disjoint subsets $S, T \subseteq B$ of size $|S|,|T| \ge \xi n$, for $\lambda < \epsilon d$ where $\epsilon$ is sufficiently small, we have
...
\begin{align*}
  \Bigl| e_G(S, T) - p|S||T| \Bigr| &\le \left| e_G(S, T) - \frac{d}{n}|S||T| \right| + \left| \frac{d}{n}|S||T| - p|S||T| \right| \\
    ...
    \ari{What is the value of $\xi$ that makes this work?. I am struggling to make this a universal constant, and $\xi = \BigO{\epsilon}$ does not work.}
...
\begin{proof}
  ...
  If $|S| \ge |U|/2$ then by a simple counting argument this trivially holds, thus we can assume $|S| < |U|/2$.  
 We claim that for every partition $X \cup Y$ of $U \setminus S$ such that $|X|, |Y| \ge |S|/3$,

 \begin{equation}
  \CutEdges{X}{Y}{G} > 1 \implies  G[U \setminus S] \text{ has at most $|S|$ connected components}  \label{eq:mini-claim}
 \end{equation}

\flushleft
To see why, assume towards a contradiction that there exists an edge between \emph{every} $X$ and $Y$ of size at least $\Size{S}/3$, \emph{and}, $G[U \setminus S]$ has more than $|S|$ connected components.
...
By construction, even if each component is singleton, we get that  $\Size{X^\star}, \Size{Y^\star} \ge \Size{S}/3$.
Now as all $C_i$'s are disjoint connected components, there can be no edge between $X^\star$ and $Y^\star$ i.e we have found a  partition without an edge between them, giving us our contradiction. \\
\flushleft
...
\end{equation}
  ...
  We will show that
    ...
    \end{equation}
    as then the minimum degree assumption on $G[U]$ implies there has to be an edge with one endpoint in $X$ and the other in $U \setminus (X \cup S) = Y$. By claim \eqref{eq:mini-claim}, the existence of such an edge implies that $G[U\setminus S]$ has at most $\Size{S}$ connected components, which would complete the proof.
