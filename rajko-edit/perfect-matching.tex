...
\ari{Why is there $X$ and $Y$ and $S$ and $T$?}
...
\begin{align*}
\left| p|S||T| - \frac{d}{n}|S||T| \right| & \le \frac{2\lambda}{b} |S||T|  
\end{align*}
...
\end{align}
This implies that $\Size{B} > n/10$, with room to spare.
}

\item{Let $B = V(G) \setminus A$. By Corollary \ref{cor:embedding_top}, $G[B]$ contains $H^{\sigma}$ such that each $\sigma(e)$ is odd. Let us denote a subgraph of $G[B]$ corresponding to $H^{\sigma}$ by $G_{\EmbeddingFunc}$.
