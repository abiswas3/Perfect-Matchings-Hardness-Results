...
\usepackage{xcolor}
\usepackage[normalem]{ulem}
%DIF ADD perfect-matching.tex   Sat Jun  7 02:38:05 2025
...
\DIFdelend \begin{corollary} 
  For every $D \in \mathbb{N}$ there exist \DIFdelbegin \DIFdel{$d_0 \in \Naturals, \epsilon \in (0,1)$}\DIFdelend \DIFaddbegin \DIFadd{$d_0 \in \Naturals, \epsilon, \alpha \in (0,1)$}\DIFaddend , such that the following holds. 
  Suppose $G$ is an $(n,d,\lambda)$-graph where $d \ge d_0$ \DIFaddbegin \DIFadd{is a constant }\DIFaddend and $\lambda < \epsilon d$. 
...
Then that the induced sub-graph $G[B]$ contains $H^{\sigma}$ }\DIFaddend and $\sigma(e)$ is odd for every $e \in E(H)$.
\end{corollary}
...
\begin{enumerate}
\item{Invoke Lemma  \ref{thm:partition} with parameters $c=0.925$ and \DIFdelbegin \DIFdel{$\epsilon = 0.025$ }\DIFdelend \DIFaddbegin \DIFadd{$\gamma = 0.025$ }\DIFaddend to get a subset $A \subseteq V(G)$ \DIFaddbegin \DIFadd{and $B = G \setminus A$, }\DIFaddend such that for every $u \in V(G)$ we have
% $u \in V$: $$ and $$
...
\DIFdelend \DIFaddbegin \DIFadd{equations \eqref{eq:size_nbr_A} and \eqref{eq:size_nbr_B}}\DIFaddend , $\Size{B} > n/20$, with room to spare and $\Size{E(G[B])} \ge nd/20$.
...
\rajko{Make references consistent. For example, all STOC papers should be listed in the same format (see the difference between [6] and [20]). Sherali-Adams should be capital, etc.}
