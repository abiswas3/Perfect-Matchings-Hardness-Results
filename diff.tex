...
\usepackage{xcolor}
\usepackage[normalem]{ulem}
%DIF ADD perfect-matching.tex   Sun Jun  8 00:33:13 2025
...
\author[1]{Ari Biswas}
%DIF 14c14
%DIF < \author[2]{Rajko Nenadov}
%DIF -------
\author[2]{Rajko Nenadov\thanks{Research supported by the Marsden Fund of the Royal Society of New Zealand.}} %DIF > 
%DIF -------
...
\providecommand{\DIFadd}[1]{\textcolor{shin-ryoku}{#1}}%DIF PREAMBLE\uwave{#1}}} %DIF PREAMBLE
\providecommand{\DIFdel}[1]{\textcolor{verylightgray}{\sout{#1}}}                      %DIF PREAMBLE\sout{#1}}}                      %DIF PREAMBLE
...
\DIFdelbegin \rajko{\DIFdel{I shortened the abstract. I think this is catchy enough, and straight to the point.}}
%DIFAUXCMD
\DIFdelend % Additionally, as Austrin and Risse do for random graphs, we show that the hardness of refuting a more general class of formulae, which includes both perfect matchings and \emph{even-colouring} formulas, also reduces to the hardness of refuting perfect matchings in spectral expanders, thereby settling an open problem of Buss and Nordstr\"{o}m.
\DIFdelbegin \rajko{\DIFdel{Add somewhere for me: ``Research supported by the Marsden Fund of the Royal Society of New Zealand.''}}
%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\rajko{\DIFdel{Please make Lemma, Corollary, and Theorem to be of the same colour as Lemma. I find Corollary yellow distracting, and Theorem is too dark.}}
%DIFAUXCMD
...
Let $\Axioms = \{ p_1 = 0, \dots, p_m = 0\}$ be a set of polynomial equations\DIFaddbegin \footnote{\DIFadd{The sum of squares proof system is a semi-algebraic proof system where $\Axioms$ may also contain inequalities of the form $p_i(x) \ge 0$. 
However, we only need equality constraints to express the existence of Perfect Matchings over graphs. 
Therefore to simplify our exposition, we write all our definitions using equality constraints only.}}\DIFaddend , which we refer to as axioms, over variables $\vec{X} = \{x_1, \dots, x_n, \bar{x}_1, \dots, \bar{x}_n\}$.
\DIFdelbegin \DIFdel{We will always assume that the axioms $x_i^2 - x_i = 0$ and $\bar{x}_i^2 - \bar{x}_i = 0$ are included in $\Axioms$ for all $i \in [m]$, which ensures that $x_i \in \bit$ for all $i\in [m]$.
...
}\DIFdelend %DIF >  We will always assume that the axioms $x_i^2 - x_i = 0$ and $\bar{x}_i^2 - \bar{x}_i = 0$ are included in $\Axioms$ for all $i \in [n]$, which ensures that $x_i \in \bit$ for all $i\in [n]$.

\DIFdelbegin %DIFDELCMD < \begin{definition}[Sum Of Squares Proofs]%%%
\DIFdelend \DIFaddbegin \begin{definition}[Sum Of Squares Refutation]\DIFaddend  Given a set of \DIFaddbegin \DIFadd{$m$ polynomial }\DIFaddend equality constraints $\Axioms$ \DIFaddbegin \DIFadd{over the reals}\DIFaddend , a Sum of Squares (SoS) \DIFdelbegin \DIFdel{proof of $h \geq 0$ from $\Axioms$ }\DIFdelend \DIFaddbegin \DIFadd{refutation }\DIFaddend is a set of polynomials $\Proof = (t_1, \dots, t_m; s_1, \dots, s_a)$ such that

\DIFdelbegin \[ \DIFdel{\sum_{i \in [m]} t_ip_i+ \sum_{i \in [a]} s_i^2 = h}\]%DIFAUXCMD
\DIFdelend \DIFaddbegin \[ \DIFadd{h \Def \sum_{i \in [m]} t_ip_i+ \sum_{i \in [a]} s_i^2 = -1}\]\DIFaddend 
\DIFaddbegin 

\DIFaddend The degree of a proof $\Proof$ is
...
\DIFdelbegin \rajko{\DIFdel{Can you briefly explain why does this prove $h \ge 0$?}}
%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < \begin{definition}[SoS Refutation]
%DIFDELCMD < %%%
\DIFdel{An SoS refutation of $\Axioms$ is a proof of $-1 \geq 0$ derived from }\DIFdelend \DIFaddbegin \DIFadd{Note that $s_i^2(x) \ge 0$ for any $x$ by definition. 
...
This chain of reasoning implies that $-1 \ge 0$, which we know is impossible. 
Therefore, the existence of the set of polynomials $\Proof$ act as a formal proof of the claim that the set of polynomials equations in }\DIFaddend $\Axioms$ \DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{is unsatisfiable.
}

\begin{definition}[Complexity Of SoS Refutation]
\DIFaddend If we let $\Pi$ denote the set of all valid SoS refutations \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend $\Axioms$, then \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{the complexity of refuting $\Axioms$ in the SoS proof system is given by 
}\DIFaddend \[ \Degree{\Axioms \SOS \bot} \Def \min_{\Proof \in \Pi}\Degree{\Proof}\]
...
\begin{enumerate}
	\item From \DIFdelbegin \DIFdel{polynomials }\DIFdelend \DIFaddbegin \DIFadd{polynomial equations }\DIFaddend $f=0$ and $g=0$ where $f,g \in \Field[\vec{X}]$ we can derive $\alpha f + \beta g = 0$ for $\alpha, \beta \in \Field$.
	\DIFdelbegin \rajko{\DIFdel{You mean polynomial equations, not polynomials?}}
	%DIFAUXCMD
\DIFdelend \item From polynomial $f=0$ where $f \in \Field[\vec{X}]$, we can derive $xf=0$ where $x \in \vec{X}$.
\end{enumerate}
...
\end{definition}
\DIFdelbegin \DIFdel{Since we always }\DIFdelend \DIFaddbegin 

\DIFadd{To ensure boolean variables, we }\DIFaddend assume the axioms \DIFdelbegin \DIFdel{contain constraints that restrict the variables to a boolean range by adding constraints }\DIFdelend \DIFaddbegin \DIFadd{$\Axioms$ always contain the equations }\DIFaddend $x_i^2 - x_i=0$ \DIFdelbegin \DIFdel{to the axioms}\DIFdelend \DIFaddbegin \DIFadd{and $\bar{x}^2_i - \bar{x}_i = 0$ for all $i \in [n]$. 
Equivalently}\DIFaddend , we can \DIFdelbegin \DIFdel{alternatively }\DIFdelend \DIFaddbegin \DIFadd{also }\DIFaddend just work in the ring $\Field[x_1, \dots, \bar{x}_n]/(x_1^2 - x_1, \dots, \bar{x}_n^2 - \bar{x}_n)$ of \DIFdelbegin \DIFdel{multilinear }\DIFdelend \DIFaddbegin \DIFadd{multi-linear }\DIFaddend polynomials.
...
\DIFaddbegin \DIFadd{Additionally, we will also assume that $1 - x_i - \bar{x}_i=0$ is also included in $\Axioms$, for all $i \in [n]$, which ensures that the bar elements are bit complements of the non-bar elements.
}\DIFaddend The following lemma is by Buss, Grigoriev, and Impagliazzo \cite{buss1999linear} and gives an instance where perfect matching is hard to refute in the worst case. 

\begin{lemma}[Worst Case Hard Instance For PC]Given any odd $n \in \Naturals$, there exists a graph $H$ with $n$ vertices and maximum degree $\MaxDegree{H}= 5$ such that Polynomial Calculus over any field of characteristic different from 2 requires degree $\Theta(n)$ to refute $\Card{H, \vec{1}}$.
\end{lemma}
...
\begin{definition}[Affine Restriction]
...
\begin{lemma}[Tutte's Generalised Criterion]
  ...
  \DIFdelbegin \[ \DIFdel{|S|f \geq \OddComponents{G[S \cup T]} + \sum_{w \in T} (f - \Size{\Neighbourhood{G}{w} \setminus S}). }\]%DIFAUXCMD
...
\begin{lemma}[Lov\`asz Local Lemma]\label{lemma:lll}
...
\DIFdelbegin \rajko{\DIFdel{$\tilde O$ is not commonly used in mathematics. We also don't care about the exact value.}}
%DIFAUXCMD
%DIFDELCMD < 
...
For $d$ sufficiently large we have $\beta \leq 1/(4d^2)$, and so $\beta d^2 \leq 1/4$. All the conditions of \nameref{lemma:lll} are satisfied, from which we conclude that, with positive probability, none of the bad events happen. This implies the desired $A \subseteq V(G)$ exists.
...
\begin{definition}[Sub-divisions]
...
\DIFdelbegin \DIFdel{We }\DIFdelend \DIFaddbegin \DIFadd{In simple words, we are given some graph $H$ with an upper bound $D$ on its maximal degree.
If we feed the embedding theorem an input parameter $D \in \Naturals$, then it can come up with constants $\alpha, \xi \in (0,1)$ and $C > 0$, such that we can topologically embed $H$ into any graph $G$ properties listed in inequality \eqref{eq:props}.
When $G$ is an $\EnDeeLambda$ graph, we will we }\DIFaddend make use of Theorem \ref{thm:embedding_top} \DIFdelbegin \DIFdel{through the }\DIFdelend \DIFaddbegin \DIFadd{to show that $G$ satisfies the required properties, thereby contains $H$ as a topological minor.
This gives us the }\DIFaddend following corollary.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \begin{corollary} 
  For every $D \in \mathbb{N}$ there exist \DIFdelbegin \DIFdel{$d_0 \in \Naturals, \epsilon \in (0,1)$}\DIFdelend \DIFaddbegin \DIFadd{$d_0, n_0 \in \Naturals, \epsilon, \alpha \in (0,1)$}\DIFaddend , such that the following holds. 
  Suppose $G$ is an $(n,d,\lambda)$-graph where $d \ge d_0$ \DIFaddbegin \DIFadd{is a constant }\DIFaddend and $\lambda < \epsilon d$\DIFaddbegin \DIFadd{, and $n \ge n_0$}\DIFaddend . 
  Let $B \subseteq V(G)$ be a subset of size $|B| \ge n/20$\DIFdelbegin \DIFdel{. Then the induced subgraph $G[B]$ contains $H^{\sigma}$, where }\DIFdelend \DIFaddbegin \DIFadd{, and }\DIFaddend $H$ is any graph with maximum degree at most $D$ and at most \DIFdelbegin \DIFdel{$\alpha n / (11 \log n)$ vertices}\rajko{\DIFdel{What is $\alpha$?}}%DIFAUXCMD
\DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{$\alpha \frac{n}{\log n}$ vertices.
Then the induced sub-graph $G[B]$ contains $H^{\sigma}$ }\DIFaddend and $\sigma(e)$ is odd for every $e \in E(H)$.
\end{corollary}


\begin{proof}
  Let $m$ denote the number of edges in the induced subgraph $G[B]$, which gives us $2m = \CutEdges{B}{B}{G}$.
Denote $b \Def \Size{B}$ and Define $p = m / \binom{b}{2}$. By the \nameref{lemma:expanders-mixing-lemma}, we have
...
\DIFaddbegin \DIFadd{In the last step we used the fact that $d$ is a constant, and that $\lambda < \epsilon d$.  This and the lower bounds on $S,T$ and $B$, we can make $\epsilon$ sufficiently small with respect to $\xi$ to get our upper bound.
    }\DIFaddend % \rajko{Feel free to expand the previous calculation and explain it in more detail.}
...
\DIFdelbegin \DIFdel{Note that $\sigma(e) \le 2 + \log n$, thus $H^\sigma$ has at most $\alpha n / 10 \le \alpha |B|$ vertices. }\DIFdelend As $G[B]$ has at least $Cn$ edges (by \nameref{lemma:expanders-mixing-lemma}), we \DIFdelbegin \DIFdel{can invoke }\DIFdelend \DIFaddbegin \DIFadd{satisfy all the constraints to invoke the }\DIFaddend \nameref{thm:embedding_top} to conclude that $G[B]$ contains $H^\sigma$.
\DIFaddbegin \DIFadd{Note that $\sigma(e) \le 2 + \log n$, and we know that $H$ has at most $\alpha \frac{n}{\log n}$ vertices. 
Thus, $H^\sigma$ has at most $\BigO{|B|}$ vertices. 
}\DIFaddend \end{proof}
...
\begin{lemma}[Perfect Matching Lemma]
Suppose $G$ is an $(n, d, \lambda)$-graph, where $\lambda < d/50$. 
\DIFdelbegin \DIFdel{If }\DIFdelend \DIFaddbegin \DIFadd{For any }\DIFaddend $U \subseteq V(G)$\DIFdelbegin \DIFdel{is of even size and }\DIFdelend \DIFaddbegin \DIFadd{, if }\DIFaddend $\delta(G[U]) \ge \cTilde$, then \DIFdelbegin \DIFdel{$G[U]$ satisfies the property of }%DIFDELCMD < \nameref{lemma:tutte-criterion}%%%
...
\begin{proof}
  \DIFdelbegin \DIFdel{Suppose }\DIFdelend \DIFaddbegin \DIFadd{Fix arbitrary }\DIFaddend $U \subseteq V(G)$\DIFdelbegin \DIFdel{is of even size and }\DIFdelend \DIFaddbegin \DIFadd{, and let }\DIFaddend $\delta(G[U]) \ge \cTilde$.
  \DIFdelbegin \DIFdel{We show that $G[U]$ satisfies the property of }%DIFDELCMD < \nameref{lemma:tutte-criterion}%%%
\DIFdel{.
  }\DIFdelend %DIF >  We show that $G[U]$ satisfies the property of \nameref{lemma:tutte-criterion}.
...
Thus, we can assume $|S| < |U|/2$.
 ...
 %DIFAUXCMD
\DIFdelend %that if $e_G(X, Y) \ge 1$ for every partition $X \cup Y$ of $U \setminus S$ such that $|X|, |Y| \ge |S|/3$,
 \begin{equation}
...
\end{align}
These steps are justified as follows: The first equation follows from the \nameref{lemma:expanders-mixing-lemma}; Equation \eqref{eq:upper-bound-X} comes from Equation \eqref{eq:upp-X} and $|X| \ge |S|/3$;
...
\DIFdelbegin \rajko{\DIFdel{Rephrased the statement of the next lemma.}}
%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{As an immediate corollary of Lemma \ref{thm:perfect-matching} we get the following corollary
}\begin{corollary}\label{cor:perfect-matching}
\DIFadd{Suppose $G$ is an $(n, d, \lambda)$-graph, where $\lambda < d/50$. 
For any even sized subset $U \subseteq V(G)$, if $\delta(G[U]) \ge \cTilde$, then $G[U]$ satisfies the }\nameref{lemma:tutte-criterion}\DIFadd{, and therefore $G[U]$ contains a perfect matching.  
}\end{corollary}

\DIFaddend \begin{lemma}[Regular Subgraph Lemma]\label{lemma:f-factor}
...
If $G' \subseteq G$ has minimum degree \DIFdelbegin \DIFdel{$\MinimalDegree{G'} \ge d - C$}\DIFdelend \DIFaddbegin \DIFadd{$\MinimalDegree{G'} \ge d - C > \frac{9d}{10}$}\DIFaddend , then $G'$ contains a spanning $f$-regular subgraph for any even $2 \le f \le d/2$.
\end{lemma}
...
\DIFdelbegin \rajko{\DIFdel{$q(S \cup T)$ should be $q(G'[S \cup T])$. Change throughout the proof.}}
  %DIFAUXCMD
\begin{align*}
\DIFdel{\OddComponents{S \cup T} }&\DIFdel{\le \Size{S}f - \sum_{w \in T} (f - \Size{\Neighbourhood{G'}{w} \setminus S}).
  }\end{align*}%DIFAUXCMD
\DIFdelend \DIFaddbegin 

  \begin{align}
    \DIFadd{\OddComponents{G' \setminus (S \cup T)} }&\DIFadd{\le \Size{S}f - \sum_{w \in T} (f - \Size{\Neighbourhood{G'}{w} \setminus S}) \label{eq:main-goal}
  }\end{align}\DIFaddend 

  As \DIFaddbegin \DIFadd{$\epsilon < \nicefrac{1}{100C^{3/2}}$ and $C > 0$, we have that $\epsilon < 1/50$.
This implies that }\DIFaddend $G$ is an $\EnDeeLambda$ graph with \DIFdelbegin \DIFdel{$\lambda < \epsilon d < d/50$ and }\DIFdelend \DIFaddbegin \DIFadd{$\lambda < d/50$.
Additionally, we are also given that }\DIFaddend $G'$ has minimal degree \DIFdelbegin \DIFdel{$\MinimalDegree{G'} \ge d - C >  11d/16$, by the Lemma above }\rajko{\DIFdel{Properly reference the lemma by its number}}%DIFAUXCMD
\DIFdel{, $G$' satisfies the  }%DIFDELCMD < \nameref{lemma:tutte-criterion}%%%
\DIFdel{, }\rajko{\DIFdel{This is not what Tutte's criterion states, so explain it}} %DIFAUXCMD
\DIFdel{that is $\OddComponents{S \cup T} \le \Size{S \cup T} = |S| + |T|$.
  Thus, }\DIFdelend \DIFaddbegin \DIFadd{$\MinimalDegree{G'} \ge d - C >  9d/10$, therefore the conditions of Lemma \ref{thm:perfect-matching} are satisfied, from which we can concluce that }$$\DIFadd{\OddComponents{G' \setminus (S \cup T)} \le \Size{S \cup T} = |S| + |T|}$$
\DIFadd{Thus, to prove \eqref{eq:main-goal}, }\DIFaddend it suffices to show
  \begin{align}
  ...
  \DIFdelbegin \rajko{\DIFdel{You need some text between these two align enviroments. Explain what is it that you want to show.}}
%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{The condition described by the inequality \eqref{eq:to-show} is satisfied via the following analysis.
...
\DIFdelbegin \DIFdel{Let }\DIFdelend \DIFaddbegin \DIFadd{For sufficiently small constant $\alpha \in (0,1)$, let }\DIFaddend $\HardInstance$ denote the graph on \DIFdelbegin \DIFdel{$h=\frac{n}{D \log n}$ }\DIFdelend \DIFaddbegin \DIFadd{$h=\alpha\frac{n}{\log n}$ }\DIFaddend vertices as given by Lemma \ref{lemma:worst-case-instance-sos} (to show lower bounds for PC, we use $H$ from Lemma \ref{lemma:worst-case-instance-PC}). Recall that any SoS proof which refutes $\PM{H}$ has degree $\BigOmega{h}$. We now make use of $H$ to show the hardness of refuting $\Card{G, \vec{t}}$. The idea is to find a restriction $\rho$ such that $\Card{G, \vec{t}}|_\rho \equiv \PM{H}$. We achieve this through the following steps.
\DIFdelbegin \rajko{\DIFdel{You have $\Gamma_G$ and $\Gamma_E$. Please use $\Gamma_G$ consistently. }}
%DIFAUXCMD
\DIFdelend 

\begin{enumerate}
\item{Invoke Lemma  \ref{thm:partition} with parameters $c=0.925$ and \DIFdelbegin \DIFdel{$\epsilon = 0.025$ }\DIFdelend \DIFaddbegin \DIFadd{$\gamma = 0.025$ }\DIFaddend to get \DIFdelbegin \DIFdel{a subset }\DIFdelend \DIFaddbegin \DIFadd{subsets }\DIFaddend $A \subseteq V(G)$ \DIFaddbegin \DIFadd{and $B = G \setminus A$, }\DIFaddend such that for every $u \in V(G)$ we have
% $u \in V$: $$ and $$

\DIFdelbegin \begin{align*} 
\DIFdel{0.9d  }&\DIFdel{\leq   \Size{\Neighbourhood{G}{u} \cap \RedA} \leq 0.95d }\\[5pt]
\DIFdel{0.05d  }&\DIFdel{\leq   \Size{\Neighbourhood{E}{u} \cap \GreenB} \leq 0.1d 
}\end{align*}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{align} 
\DIFadd{0.9d  }&\DIFadd{\leq   \Size{\Neighbourhood{G}{u} \cap \RedA} \leq 0.95d \label{eq:size_nbr_A}}\\[5pt]
\DIFadd{0.05d  }&\DIFadd{\leq   \Size{\Neighbourhood{G}{u} \cap \GreenB} \leq 0.1d 
\label{eq:size_nbr_B}
}\end{align}\DIFaddend 
}

\item{\DIFdelbegin \DIFdel{Let $B = V(G) \setminus A$. }\rajko{\DIFdel{$B$ should be defined earlier.}} %DIFAUXCMD
\DIFdelend From \DIFdelbegin \DIFdel{the partition lemma above }\rajko{\DIFdel{Not the partition lemma itself, but (25) and (26)}}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{equations \eqref{eq:size_nbr_A} and \eqref{eq:size_nbr_B}}\DIFaddend , $\Size{B} > n/20$, with room to spare and \DIFdelbegin \DIFdel{$\Size{E(G[B])} \ge nd/20$}\DIFdelend \DIFaddbegin \DIFadd{$\Size{E(G[B])} \ge nd/20 = Cn$}\DIFaddend .
    \DIFdelbegin \DIFdel{$G[B]$ }\rajko{\DIFdel{Why is $G[B]$ a sentence on its own?}}%DIFAUXCMD
\DIFdel{.
    }\DIFdelend By Corollary \ref{cor:embedding_top}, $G[B]$ contains $H^{\sigma}$ such that each $\sigma(e)$ is odd. Let us denote a subgraph of $G[B]$ corresponding to $H^{\sigma}$ by $G_{\EmbeddingFunc}$.
...
\item{ As $n$ is odd and $\Size{\Vertices{G_{\EmbeddingFunc}}}$ is odd, we have that $U = \Vertices{G} \setminus \Vertices{G_{\EmbeddingFunc}}$ has even size. From \eqref{eq:size_nbr_A} we have that $G[U]$ has minimum degree at least $\cTilde$. As $\lambda < d/50$, we can invoke \DIFdelbegin \DIFdel{the }%DIFDELCMD < \nameref{thm:perfect-matching} %%%
\DIFdelend \DIFaddbegin \DIFadd{Corollary \ref{cor:perfect-matching} }\DIFaddend to conclude $G[U]$ has a perfect matching $M$. 
}

\item{
  Consider the subgraph $G' \subseteq G$ obtained by deleting all edges $e \in \Edges{G_{\EmbeddingFunc}} \cup M$, where $M$ is the perfect matching from the step above. As $\MaxDegree{H} \leq 5$, every vertex $u \in G$ loses at most $5$ edges in this process. Thus, we have $\MinimalDegree{G'} \ge d - 5$. As $\lambda < \epsilon d$, by \nameref{lemma:f-factor} we have that \DIFdelbegin \DIFdel{$G$ }\DIFdelend \DIFaddbegin \DIFadd{$G'$ }\DIFaddend contains a $(t-1)$-regular spanning subgraph \DIFdelbegin \DIFdel{$G'$}\DIFdelend \DIFaddbegin \DIFadd{$G''$}\DIFaddend . \DIFdelbegin \rajko{\DIFdel{The last sentence is wrong. You want to say that $G'$ contains a $(t-1)$-regular spanning subgraph. Consequently, the definition of $\rho$ is not correct.}}%DIFAUXCMD
\DIFdelend }
...
%DIFDELCMD < %%%
\rajko{\DIFdel{Make references consistent. For example, all STOC papers should be listed in the same format (see the difference between }%DIFDELCMD < [%%%
\DIFdel{6}%DIFDELCMD < ] %%%
\DIFdel{and }%DIFDELCMD < [%%%
\DIFdel{20}%DIFDELCMD < ]%%%
\DIFdel{). Sherali-Adams should be capital, etc.}}
%DIFAUXCMD
\DIFdelend 
