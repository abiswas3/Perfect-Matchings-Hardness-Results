...
%DIF DEL old.tex   Tue Oct 21 02:38:23 2025
%DIF ADD new.tex   Tue Oct 21 02:38:38 2025
...
%DIFDELCMD < %%%
\DIFdelend \begin{definition}[Complexity Of SoS Refutation]
...
}\DIFdelend \DIFaddbegin \DIFadd{lemma is by \mbox{%DIFAUXCMD
\citep[Lemma 4.3]{Austrin_2022}}\hskip0pt%DIFAUXCMD
...
%DIF <  \ari{It would be nice if you could write a short paragraph about why this theorem is true. 
%DIF <  I know they can go find the original reference, so if you think this would be easy then please add something. 
%DIF <  }
%DIF <  \rajko{What do you mean ``why this theorem is true''? You mean the intuition? That I can't give in one paragraph. If you mean why does it follow from [11, Theorem 1], that is rather straightforward and needs no explanation.}
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \begin{theorem}[Embedding Theorem] \label{thm:embedding_top}
...
\DIFdelbegin \begin{align*}
  \DIFdel{\bigl| e_{G}(S, T) - |S||T|p \bigr| \le \xi |S||T| p %DIFDELCMD < \label{eq:props}%%%
}\end{align*}%DIFAUXCMD
...
As $G[B]$ has at least $Cn$ edges (by \DIFaddbegin \DIFadd{the }\DIFaddend \nameref{lemma:expanders-mixing-lemma}), $\sigma(e) \le 2 + \log n$, and $H$ has at most $\alpha \frac{n}{\log n}$ vertices, we can invoke \DIFaddbegin \DIFadd{the }\DIFaddend \nameref{thm:embedding_top} to conclude that $G[B]$ contains $H^\sigma$.
\end{proof}
...
Thus, we can assume $|S| < |U|/2$.
 ...
 %that if $e_G(X, Y) \ge 1$ for every partition $X \cup Y$ of $U \setminus S$ such that $|X|, |Y| \ge |S|/3$,
 \begin{equation}
...
\DIFaddbegin \DIFadd{In the lemma below, the parameter $C$ should be read as the subgraph $G'$ obtained by deleting a constant number of edges from the $\EnDeeLambda$ graph $G$.
}\DIFaddend 
...
\begin{align}
  \DIFdelbegin %DIFDELCMD < \CutEdges{S}{T}{G} %%%
...
%DIFDELCMD < $ \frac{\Size{S}}{C+3} < \Size{T} < \Size{S}$. As $\Size{S} + \Size{T} < n$, this assumption implies that $\Size{S} < n\left(1 - \frac{1}{C+3}\right)$.
%DIFDELCMD < Now using the \nameref{lemma:expanders-mixing-lemma}
%DIFDELCMD < 

%DIFDELCMD < \[ \CutEdges{S}{T}{G'} \le \frac{d}{n}\Size{S}\Size{T} + \epsilon d \sqrt{\Size{S}\Size{T}} \le \frac{(C-1)d}{D}\Size{T} + \epsilon d \Size{T}\sqrt{C} < \Size{T}(d-c)\]
%DIFDELCMD < 

%DIFDELCMD < which gives us Equation \eqref{eq:to-show-3}.
%DIFDELCMD <   }
%DIFDELCMD <   %%%
...
\begin{enumerate}
\DIFdelbegin %DIFDELCMD < \item{Invoke Lemma  \ref{thm:partition} with parameters $c=0.925$ and $\gamma = 0.025$ to get subsets $A \subseteq V(G)$ and $B = G \setminus A$, such that for every $u \in V(G)$ we have
%DIFDELCMD < % $u \in V$: \begin{displaymath} and \end{displaymath}
%DIFDELCMD <     

%DIFDELCMD < \begin{align*} 
%DIFDELCMD < 0.9d  &\leq   \Size{\Neighbourhood{G}{u} \cap \RedA} \leq 0.95d \label{eq:size_nbr_A}\\[5pt]
%DIFDELCMD < 0.05d  &\leq   \Size{\Neighbourhood{G}{u} \cap \GreenB} \leq 0.1d 
%DIFDELCMD < \label{eq:size_nbr_B}
%DIFDELCMD < \end{align*}
%DIFDELCMD < }
%DIFDELCMD < %%%
...
\DIFdelbegin %DIFDELCMD < \item{From equations \eqref{eq:size_nbr_A} and \eqref{eq:size_nbr_B}, $\Size{B} > n/20$, with room to spare and $\Size{E(G[B])} \ge nd/20 = Cn$.
%DIFDELCMD <     By Corollary \ref{cor:embedding_top}, $G[B]$ contains $H^{\sigma}$ such that each $\sigma(e)$ is odd. Let us denote a subgraph of $G[B]$ corresponding to $H^{\sigma}$ by $G_{\EmbeddingFunc}$.
%DIFDELCMD <     We can describe $G_\EmbeddingFunc$ as a function $\EmbeddingFunc: V(H) \to B$ together with a collection of pairwise vertex-disjoint (other than at the endpoints) paths  $\Path{\Embedding{u}}{\Embedding{v}}$ in $G[B]$, for $(u,v) \in E(H)$.
%DIFDELCMD <     Observe that it is at least as hard to refute\footnote{Note that this by itself does not guarantee it is hard to refute $\PM{G}$. We need item (3) and this to show hardness in refuting $\PM{G}$.} $\PM{G_{\EmbeddingFunc}}$ as it is to refute $\PM{H}$.
%DIFDELCMD < To see why, let $y_1, \dots, y_{E(H)}$ denote the variables for the $\PM{H}$ formulae for each edge of $H$.
%DIFDELCMD < We use as shorthand $\mathcal{Y} = (y_e)_{e \in E(H)}$ and $\Complement{\mathcal{Y}} = (\Complement{y}_e)_{e \in E(H)}$.
%DIFDELCMD < Define a mapping $\rho': \Edges{G_\EmbeddingFunc} \to \{0,1, \mathcal{Y}, \Complement{\mathcal{Y}} \}$ as follows. For each $(u,v) \in E(H)$, let $\rho'(x_e) = y_{u,v}$ where $e$ is the first edge on the path $\Path{\Embedding{u}}{\Embedding{v}}$.
%DIFDELCMD < Map each variable $x_e$ for $e \in \Path{\Embedding{u}}{\Embedding{v}}$ alternately to $y_{u,v}$ or $\bar{y}_{u,v}$, such that the edges of the path adjacent to $\Embedding{u}$ and $\Embedding{v}$ are set to $y_{u,v}$. This is always possible as $\Path{u}{v}$ has odd length. Observe that $\PM{G_\EmbeddingFunc}|_{\rho'} \equiv \PM{H}$. 
%DIFDELCMD <   }
%DIFDELCMD < %%%
...
\DIFdelbegin %DIFDELCMD < \item{
%DIFDELCMD <   Consider the subgraph $G' \subseteq G$ obtained by deleting all edges $e \in \Edges{G_{\EmbeddingFunc}} \cup M$, where $M$ is the perfect matching from the step above. As $\MaxDegree{H} \leq 5$, every vertex $u \in G$ loses at most $5$ edges in this process. Thus, we have $\MinimalDegree{G'} \ge d - 5$. As $\lambda < \epsilon d$, by \nameref{lemma:f-factor} we have that $G'$ contains a $(t-1)$-regular spanning subgraph $G''$. }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \item{
