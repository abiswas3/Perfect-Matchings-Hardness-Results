...
\usepackage{xcolor}
\usepackage[normalem]{ulem}
%DIF ADD perfect-matching.tex   Fri Jun  6 19:21:05 2025
...
\rajko{Can you briefly explain why does this prove $h \ge 0$?}
...
can be found in any textbook on randomised algorithms (for example, see \citep[Chapter 1, Chapter 7]{mitzenmacher2017probability}). The following is the main result of this section. \rajko{$\tilde O$ is not commonly used in mathematics. We also don't care about the exact value.}
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \ari{Did you already remove the $\tilde{O}$?}
...
\begin{corollary} 
  For every $D \in \mathbb{N}$ there exist $d_0 \in \Naturals, \epsilon \in (0,1)$, such that the following holds. Suppose $G$ is an $(n,d,\lambda)$-graph where $d \ge d_0$ and $\lambda < \epsilon d$. Let $B \subseteq V(G)$ be a subset of size $|B| \ge n/20$. Then the induced subgraph $G[B]$ contains $H^{\sigma}$, where $H$ is any graph with maximum degree at most $D$ and at most $\alpha n / (11 \log n)$ vertices \rajko{What is $\alpha$?}, and $\sigma(e)$ is odd for every $e \in E(H)$.
\end{corollary}
